...
Profiler deals with them.

Cross Site Request Forgery (CSRF)
---------------------------------

Profiler stores authentication information in HTML5 local storage — not in
cookies. By avoiding cookies, Profiler side steps the issue of CSRF completely.
...
Due to the use of Angular.dart, Profiler mostly side steps XSS concerns.
...
our mitigation strategy if we do find a need to use this risky directive.
Profiler already has a very simple and restrictive ``NodeValidatorBuilder``
instantiated in main.dart that will be used by default for all ``NgBindHtml``
directives.

SQL Injection (sqli)
--------------------

Profiler uses the `SQL Alchemy ORM <http://docs.sqlalchemy.org/>`__ to provide a
