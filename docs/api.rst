...
The REST API is a crucial part of the Profiler architecture. Since Profiler
doesn't render any user interface on the server side, the API must expose all of
the functionality that the client could possibly need. This has the positive
side benefit of making it easy to write additional clients for Profiler. For
example, we could write an iOS or Android client without needing to refactor the
server code.

The other obvious benefit of a REST API, of course, is that it simplifies data
sharing with other applications. Other applications can put data into Profiler
and/or retrieve data back out of Profiler, which gives us a lot of flexibility.

This document explains the basic design and principles of the REST API.
...
.. important::
    ...
    URL like ``/account/1`` might trigger an API request like
    ``/api/account/1``. There's no reasonable way for our Dart client to
    discover URLs in this scenario. Nonetheless, we should still ensure that our
    API provides URLs to the client, even if the Dart client rarely needs them.
    It makes debugging faster and easier and for certain types of clients it can
