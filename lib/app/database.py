...
def make_date_columns(date_column, start_date, end_date, delta, unit):
    ...
    COUNT(*) and group by truncated dates, but any month
    where the count is zero will be omitted from the results.
    ...
    ``start_date`` is the date at which to start making columns. IMPORTANT:
        this date needs to be truncated to the same precision as ``unit``:
        e.g. if ``unit`` is ``month``, then ``start_date``
        should have days set to 1,
    ...
    ``delta`` is a ``relativedelta`` object which defines the timespan covered
        by each column.
    ``unit`` is any Postgres date_trunc unit, e.g. ``week``,
        ``month``, ``year``, etc.
        See:
